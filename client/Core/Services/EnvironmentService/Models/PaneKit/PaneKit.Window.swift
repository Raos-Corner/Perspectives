...
extension PaneKit {
    class Window: NSObject, Identifiable, NSWindowDelegate {
        ...
        private var main: PaneAppWindow? = nil
        ...
        init(id: String,
             useMain: Bool,
             size: CGSize = .init(840, 480),
             maxSize: CGSize = .init(840, 840),
        ...
        func build(title: String? = nil, center: Bool = false) {
            DispatchQueue.main.async { [weak self] in
                self?.main = PaneAppWindow(self?.size ?? .zero, title: title, center: center)
        ...
        func bringToFront() {
            guard main?.isVisible == true else { return}
            main?.level = .floating
            main?.makeKeyAndOrderFront(nil)
            main?.level = .normal
            windowVisibility.open(id: self.id)
...
class PaneAppWindow: NSWindow {
    
    private var lastPoint: CGPoint? = nil
    
    init(_ size: CGSize, title: String? = nil, center: Bool = false) {
        
        let origin = PaneAppWindow.originPoint(size, newSize: .zero, center: center)
